...
class image_converter:
  ...
  def __init__(self):
    ...
    self.command_sub = rospy.Subscriber("/cam_vel", Twist, self.Commandcallback)
  ...
  def callback(self,data):
    ...
    try:
      ...
      print "Failed 1st Pose estimator"
    ...
    for c in cnts:
      ...
      if len(approx) >= 4 and len(approx) <= 6:
        ...
        if keepDims and keepSolidity and keepAspectRatio:
          ...
          if success:
          #print cX
            self.twist.linear.x = self.translate(translation_vector[2], 55 ,200 ,-.1, 1 )
            self.twist.linear.y = 0
            self.twist.linear.z = 0
            self.twist.angular.x = 0
            self.twist.angular.y = 0
            self.twist.angular.z = self.translate(cX, 70 ,500,-1, 1 )
  ...
  def detect_circle(self, frame):
      ...
      gray = cv2.adaptiveThreshold(gray, 180, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 3.5)
      ...
      try:
          ...
          print "Failed to apply HoughCircles"
      ...
      try:
          self.twist.linear.x = self.translate(r, 55, 200, -.1, 1)
      except:
