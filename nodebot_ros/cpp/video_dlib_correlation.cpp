...
int main ( int argc, char **argv ) try
{
	...
	Mat qr,qr_raw,qr_gray,qr_thres,qr_thres_prev;
	...
	while(key != 'q')				// While loop to query for Image Input frame
	{
		...
		qr_thres_prev = Mat::zeros(100, 100, CV_8UC1);
		...
		cerr << " Inside else , correlationg tracker activated" << endl;
		Mat immg;
		immg = imread("sample.jpg",1);
		//cv::Mat temp,image_temp;
		dlib::cv_image<dlib::bgr_pixel> temp(immg);
		dlib::cv_image<dlib::bgr_pixel> image_temp(image);		
		//cv_image<bgr_pixel> qr_thresh(temp);	
		//cv_image<bgr_pixel> image(image_temp);
		
		tracker.start_track(temp, dlib::centered_rect(dlib::point(100,100), 0, 0));
		tracker.update(image_temp);
		
                Mat image_tracking = dlib::toMat(image_temp);
		//imshow ( "Image", dlib::toMat(temp) );		
		imshow ( "Image1", immg);		
		}
